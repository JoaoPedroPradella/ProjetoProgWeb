...
$json = file_get_contents("http://192.168.0.209/ERP/json/vendas_json.php?token=$token");
...
$sql_select = 'SELECT id_venda FROM vendas WHERE id_venda = ?;';

$sql_update = 'UPDATE vendas SET cliente_id = ?, data_venda = ?, valor_total = ?, status = ? WHERE id_venda = ?;';

$sql_insert = 'INSERT INTO vendas (id_venda, cliente_id, data_venda, valor_total, status) VALUES (?, ?, ?, ?, ?);';
...
foreach ($obj as $key => $value) {
    ...
    if (empty($dados)) {
        $params = [
            ...
            $data_venda[$i] = $value->data_venda,
            $valor_total[$i] = $value->valor_total,
